...
Möglichkeit das ein Prozess auf unterschiedlichen Ressourcen ausgeführt
...
Simulation. Dabei geht leider der Bezug zur Architektur (z.\,B. Hardwarekomponenten) %%todo
 verloren. 
...
Für die Simulation des InfiniBand-HCA wurde dieses in 17 separate Prozesse 
zerlegt (vgl. Abbildung \ref{fig:hca_func_units_lib} auf Seite 
\pageref{fig:hca_func_units_lib}). Jeder dieser Prozesse
 stellt einen Problemgraphknoten $v \in V_P$dar.
...
Eine Komponente stellt die ausführende Umgebung für ein Prozess dar. Einen
...
sich.\footnote{Die einzelnen Ausführungszeiten eines Prozesses muss für alle 
...
\code{compute}-Funktion mit \code{return} zurück und das Problem-Modul kann
...
\subsection{Resourcen-Konfigurationsdatei}\label{resconfig}

Die zu verwendende Allokation und Bindung für eine Simulation wir in Form
...
Priorität\footnote{Wird nur für prioritätsbasietes Scheduling 
...
verwendet. Die Ablaufplannung auf Ressource \emph{HWM1} unterliegt der
\emph{First-Come-First-Serve}-Stra\-te\-gie. Ein \emph{Round-Robin}-Verfahren
mit einer Zeitscheibe von 200 ns dient als Ablaufplannungsstartegie auf
...
von Ablaufplannungsstrategien. Eine detaillierte Beschreibung bezüglich der
...
Dabei wird für jede Ressource eine Datei ereugt, z.\,B.
...
Umständen eine mehrfache Betrachtung des gleichen Zeitpunkts notwendig.

Dieses Simulationskonzept führt dazu das die Zeit nicht Schritt für Schritt
...
abarbeiten. Die ereignisgesteuerte Simulation sorgt dafür das zu dem 
...
ein Datenpaket für den HCA-2, diese kommt dort jedoch in der Vergangenheit an.

Diese Problem resultiert aus der \emph{verteilten Event-Queue} der getrennten
...
\item Anpassung des SystemCQuellcodes, um verteilte Event-Queues zu unterstützen.
...
SystemC-Installationen. Das Kosten-Nutzen-Verhältniss steht dann, auch 
...
Paket bei ihm ankommen muss. Wegen dem Wissen das ihm HCA-1 neue Pakete frühestens zu selben
oder zu späteren Zeiten produziert werden können, kann sich der paketlesende Prozess
...
Zeit ``schenken''\footnote{Wegen der Verögerung kann garantiert werden, dass eine Simulation
...
ausgetauscht wird.
\item[timestamp:] Ein Zeitstempelpaket, das dazu dient dem Empfänger Zeit zu garantieren.
\end{description}
