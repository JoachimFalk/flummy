...
#ifdef VPC_DEBUG
      std::cerr << "LCBPE> 'operator >' bound_count compare " << this->bound_count << " < " << lcbPE.bound_count << std::endl;
      std::cerr << "or id compare " << this->id << " < " << lcbPE.id << std::endl;
#endif //VPC_DEBUG
      
      if(this->bound_count < lcbPE.bound_count || (this->bound_count == lcbPE.bound_count && this->id < lcbPE.id)){
        return true;
      }
