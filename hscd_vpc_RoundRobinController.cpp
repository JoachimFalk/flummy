...
namespace SystemC_VPC{
   ...
   std::deque<ProcessControlBlock* >& newTasks)
  {
...
#ifdef VPC_DEBUG
        std::cerr << VPC_YELLOW("RoundRobinController "<< this->getName()
                  << "> timeslice elapsed at: ") << sc_time_stamp().to_default_time_units()
                  << endl;
#endif //VPC_DEBUG
    ...
    if(this->scheduledConfiguration != NULL){
      ...
      std::cerr << VPC_YELLOW("RoundRobinController "<< this->getName()
                << "> timeslice lasts: "
                << this->TIMESLICE-(sc_time_stamp().to_default_time_units()-this->lastassign)
                << " at: ") << sc_time_stamp().to_default_time_units() << endl;
#endif //VPC_DEBUG
  
      this->waitInterval =
        new sc_time( this->TIMESLICE-(sc_time_stamp().to_default_time_units()-this->lastassign),
                     true);
    }
  }
  ...
  void RoundRobinController::signalDeallocation(bool kill){
    this->remainingSlice =
      this->remainingSlice - (sc_time_stamp().to_default_time_units() - this->lastassign);
  }
