...
#define S_RUNNING 'R'
  ...
  struct Tracing{
    Tracing() :  traceSignal(new sc_signal<trace_value>()), lastChange(SC_ZERO_TIME), lastValue(0) {}
    ...
    public:
    /**
     * Set the valur for tracing.
     * If the signal is identic to lastValue then the ascii bit for lowercase id toggled
     */
    void value(trace_value value){
      if(lastChange != sc_time_stamp()){ // remember value from last real changing (ignore delta cycle changing)
	lastValue    = *traceSignal;
	lastChange   = sc_time_stamp();
      }
      if(lastValue == value){ // if value does not change toggle between upper and lower case
